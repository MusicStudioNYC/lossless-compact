...
| 2026-09-20 | Flip `MusicStudioNYC/lossless-compact` from private to public (`gh repo edit --visibility public --accept-visibility-change-consequences`) | **Done 2026-09-20** (public; secrets and username scan clean) |
...
| 2026-09-20 | Interactive check of the `turn.complete` auto-compaction trigger in Cursor (a chat past 120k tokens; expect `command_lifecycle` + a `[lossless-compact]` note, no minute-long summary) | **Finding 2026-09-20:** the extension runs every chat on the SDK path, where the host refuses `$.session.compact()` (`auto-compact skipped (... not available in a headless (-p / SDK) session yet ...)` in the exthost log), so until today the 120k trigger never compacted anything there. Now it runs `$.command.run({ command: 'compact' })` instead, verified headless (`turn.complete` -> `/compact` -> the plugin's `session.compact`); the in-extension run past 120k is still to be seen |
...
| 2026-09-20 | Codex edition: (1) read-only Codex JSONL transcript importer beside `src/node/transcripts.ts`, run a real session through `optimize()` to prove the core is host-portable; (2) archive-and-retrieve adapter around Codex's native compaction (snapshot on PreCompact, archive on PostToolUse, retrieve on UserPromptSubmit) | Owner chose to ship Claude Code first (2026-09-20). Codex's PreCompact reportedly cannot return replacement messages (unverified), so it is a different, weaker promise than the Claude plugin: release as its own honestly-labelled edition, never as a launch claim |
| 2026-09-20 | Cursor edition: would need its own postToolUse sidecar ledger from session start (the chat file reportedly has no tool-result records and no stable call ids; preCompact is observational) | Reduced capability; only after Codex, and only if Cursor hooks gain compaction replacement or per-prompt context injection |
| 2026-09-20 | A skipped auto-compaction (too little to remove) leaves the records `optimize()` already archived in `.lossless-compact/archive/` although the content is still live; `/lossless list` shows them | Harmless (restore duplicates content that is still there); would need `optimize()` to defer archive writes until the result is accepted |
