# Handoff — lossless-compact build, 2026-09-20

**Renamed 2026-09-20:** `context-os` → `lossless-compact` (GitHub repo, plugin, marketplace, npm name, archive folder `.lossless-compact/`, hook file, config identifiers). Rationale in "Naming decision" at the end of this file.

**Status:** open. Successor: mark this done when Phase 3 (durable memory) lands.
**Repo:** the `fast-jev-compaction` checkout under `Dropbox\Websites` (the folder keeps the fork's original name; branch `main`, clean; the smoke-test chat of 2026-09-20 added commits `2812cae`…`969b482` plus this update — see `git log`).
**Deployed vs local:** pushed to https://github.com/MusicStudioNYC/lossless-compact (**private** for now; `origin`, branch `main`; `gh` 2.101 is installed at `C:Program FilesGitHub CLIgh.exe` and logged in as MusicStudioNYC). No npm publish. On this PC the plugin is **installed at user scope** (`claude plugin install lossless-compact@lossless-compact`, marketplace source = this checkout as a directory, so the host reads `hooks/hooks.json` from the repo in place and edits go live on the next session); `~/.claude/settings.json` `env` carries `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1` and `TYPESAFE_API_KEY` (backup: `settings.json.bak-2026-09-20`). The plugin **has** run live in Claude Code 2.1.278 (headless `-p`; see "Live smoke test" below and docs/evals.md).
...
- Auto-compaction triggers on an absolute count: `compactAtTokens` (default 120 000) in `hooks/lossless-compact.ts` `shouldCompact`; `compactAtPercent` is off unless set.
- A bare `/context` calls `next(event)` so Claude Code's built-in usage grid still shows, then gives a short active/classifier toast; `/context status` and the other subcommands are ours. The first headless check falsely suggested appended text would render under the grid, but interactive Claude Code uses a native modal and discards that text. (The built-in command exists — registering the same name is refused, while the `command.run` hook still intercepts it.)
- Settings UX, owner asked for options: recommended A (plugin `userConfig` + `/config`, already there) now, B (per-project `.lossless-compact/config.json` + `/context set <key> <value>`, precedence project › /config › defaults, `/context` shows effective values) as the next small step; C (a plugin-drawn settings panel via `ui.render`) only if the product gets users. Owner's choice pending.

## Live smoke test (done 2026-09-20, second chat)

Driven headless. `claude.exe` 2.1.278 is at `%USERPROFILE%\.cursor\extensions\anthropic.claude-code-2.1.278-win32-x64\resources\native-binary\claude.exe` (PATH still has 2.1.238). Driver per turn: `claude -p --session-id <uuid> --plugin-dir . --model sonnet --output-format stream-json --verbose --include-hook-events --allowedTools Read,Glob,Grep --debug-file <log> "<prompt>"`, then `--resume <uuid>`; env `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1`, `TYPESAFE_API_KEY=…` and **`MSYS_NO_PATHCONV=1`** (Git Bash otherwise turns `/compact` into `C:/Program Files/Git/compact`). The hook's `$.ui.log` lines land in the debug file as `[lossless-compact] $.ui.log: …`. The session (`fb138cff-7114-4821-846d-8b72fc4b7d84` under `~/.claude/projects/<encoded checkout path>/`) and its `.lossless-compact/` archive are still on disk.

Verified: the plugin loads and validates on 2.1.278; `session.compact` returned our 19 messages for a 258k-token transcript in 819 ms (Jev 400 ms) and the host logged "a hook's 19 messages stand; core never ran"; archive index + shard and the 1 MB snapshot were written; the note is message 2 and the raw `.jsonl` path is found from inside the sandbox (`$.fs.exists` works on absolute paths outside the project); `/context status`, `list`, `why`, and `restore` answer in ~10 ms; `prompt.submit` gets `event.text`, retrieval ran in 126 ms and the model answered from the `<retrieved_context>` block and said so. The bare `/context` claim from that headless run was corrected after the owner's interactive screenshot: its appended text does not render in the native modal, so v0.4.1 preserves the grid and uses a toast. The 10 s hook budget is not a constraint (the host waited 135 s for a compaction that included its own summary).
...
Still open: **the `turn.complete` auto-trigger could not be exercised** — `$.session.compact()` throws "not available in a headless (-p / SDK) session yet"; it needs one interactive session that crosses 120k tokens. `/context restore` returning `context: [block]` was only seen as text (a command in `-p` runs no model turn). The keep-nothing review's `$.model.fork` path and its `$.ui.ask` dialog are interactive-only too (headless: fork is cold, ask rejects); the haiku path ran live.

## Interactive `/compact` report (2026-09-20, third chat)

The owner ran `/compact` in a VS Code chat tab on a kosher.chat session and it "loaded forever" (cancelled after 76 s). The Cursor extension log (`%APPDATA%\Cursor\logs\<window>\exthost\Anthropic.claude-code\Claude VSCode.log`; one `claude.exe` process per chat tab, lines prefixed `From claude:`) shows that tab's process was launched at 03:11Z, before the plugin was installed: `installed plugins' hooks modules not loaded … Registered 0 hooks from 0 plugins`, then `[API REQUEST] /v1/messages source=compact` — Claude Code's own summary of a 207k-token context, first byte after 13 s, still streaming when cancelled. `/reload-plugins` (run in a different tab) logs `hooks modules unchanged, kept as loaded`; it does not load a module into a running process either. The same transcript, copied under a new id and resumed headlessly through 2.1.278 with the plugin (`C:\Temp\ctxos-repro`): `compact_boundary pre_tokens 207854 → post_tokens 23397, duration_ms 991`, 115 records archived, snapshot written. README step 3 now says so. Item 1 below still needs a tab opened after the install.

Later the same day, from the log of a compaction in this checkout: Jev ran (two `POST …/v1/systemone`, 579 and 651 ms; 894 ms end to end; `core never ran`), but `$.ui.toast` logged `no notification bar in a headless session` — the extension runs Claude Code headless, so **toasts never render in VS Code/Cursor**. The compaction note now carries a `Classifier: … · tokens · messages · ms` line instead. And the command was renamed: **`/context <sub>` → `/lossless <sub>`** (owner's choice; the built-in name was refused by the host, so the subcommands were an invisible hijack that never appeared in the typeahead; `/lossless` is registered and listed). The "Kept as is: the `/context` command" decision below is superseded. The VS Code/Cursor extension fills its slash menu from a one-time startup snapshot (`claudeConfig.commands`, even served from cache), so a runtime `$.command.register` never appears there although the host logs `/lossless listed`; skills on disk (`/ship`) do. Fix: `commands/lossless.md` in the plugin puts `lossless` (and the alias `lossless-compact:lossless`) in the startup list and the `command.run` hook still answers it (verified headless: init `slash_commands` has both, output is the status text). Codex work: see `2026-09-20-codex-edition-handoff.md`.
...
- **npm package name unchanged** (`fast-jev-compaction`), plugin renamed `lossless-compact`: the owner has not said where this will be published; see open questions.

## Owner's answers from the chat

- Repo & name: "Push to <you>/lossless-compact, keep npm name for now" → done 2026-09-20 (third chat): `MusicStudioNYC/lossless-compact`, private until the interactive check passes.
...
- Mid-chat: "what's the smartest way to make settings available to everyone … GUI if simple" → options A–D laid out; owner chose **B** (per-project `.lossless-compact/config.json` + `/context set`).
...
- `hooks/fast-jev.ts` is not registered in `hooks.json` any more but must stay: `lossless-compact.ts` imports its helpers (`toSessionMessages`, `jevAsker`, `resolveHookConfig`, `summarize`, `decisionLogLines`).
...
- The upstream #53 guard (`suspectCalibration`: ≥ 5 scored, none kept) no longer falls straight back: `reviewKeepNothing` asks `$.model.fork` (the session's model over its own transcript), then `$.model.complete` (haiku, user turns quoted), then `$.ui.ask` (remove / remove and don't ask again this session, stored under `lossless-compact:trust:<session>` / Claude's summary); headless with no reviewer → built-in summary with the note. Owner's design ("ask an LLM to review what just happened … or surface to the user"). The `fork` path has only been unit-tested: headless it is always cold. A score band was tried first and dropped: the real #53 case was everything under 0.3 with a 0.5 threshold, which a band cannot tell from a disposable session.
- **`rechain`** (`hooks/lossless-compact.ts`): every message handed back to the host is stripped of its engine handle. Without this, on `--resume` Claude Code 2.1.278 walks `parentUuid` links from the newest message back into the pre-boundary log and the compaction is undone (seen live: the next `/compact` saw every archived read again, twice, `ambiguous_structure=18`). Cost: thinking-only assistant messages are dropped (they would be stored as "(no content)"). Do not "optimise" this away by keeping handles on untouched messages: any handle-kept message *after* an evicted one re-opens the old chain.
...
- Patching `hooks/lossless-compact.ts` through `node - <<EOF` heredocs mangles template literals and `\b`; use the Edit tool for multi-line TypeScript changes.
...
`context-os` → **`lossless-compact`**. Owner's call after a chat that weighed the options; the reasoning, so nobody reopens it:
...
- Kept as is: the `/context` command (worked in the smoke test; renaming it would widen the change for no gain).
- Same chat: Codex and Cursor editions deferred — owner chose to ship Claude Code first; both are in `docs/follow-ups.md` with what each host can and cannot do.
